...
class extractor():
    @staticmethod
    def extract_data(agent_init,agent_final,config,local_dir,skip = 1,data='episodes'):
        ...
        if data=='episodes':
            for i in range(agent_init,agent_final,skip):
                ...
                filename = os.path.join(local_dir,'episodes',str(agent)+".json")
        ...
        if data == 'difficult':
            for i in range(agent_init,agent_final,skip):
                ...
                #set location of data
                trialpath = os.path.join(config['difficult_dir'],str(agent))
                file_list = [name for name in os.listdir(trialpath) if os.path.isfile(os.path.join(trialpath,name))]

                #select episodes 
                for i in tqdm(range(0,len(file_list))):
                    with open(os.path.join(config['difficult_dir'],str(agent),file_list[i])) as file:
                        state_tensor = json.load(file)
                        states, _, actions, rewards = zip(*state_tensor)
                        states_position = []
                        for i in range(len(states)):
                ...
                filename = os.path.join(local_dir,'difficult',str(agent)+".json")
                with open(filename,"w") as file:
                    file.write(json.dumps(episodes,indent=0)) #save state_tensor for agent optimization
